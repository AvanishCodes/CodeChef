...
bool checkString(string s){
    bool status[4]; // 0 for small, 1 for captial, 2 for special, 3 for number
    for(int i=0; i<4; i++)
        status[i] = false;
    if(s.length() < 10 )
        return false;
    // || (s[0] >= 65 && s[0] <= 95) || (s[s.length()-1] >= 65 && s[s.length()-1] <= 90) || s[0] ==  '@' || s[0] == '#'|| s[0] == '%'|| s[0] == '&'|| s[0] == '?' || s[s.length()-1] ==  '@' || s[s.length()-1] == '#'|| s[s.length()-1] == '%'|| s[s.length()-1] == '&'|| s[s.length()-1] == '?'
    else
